...
void geraMatrizCentralidades2(float to[][dimMatriz], igraph_vector_t* from, int dim, int col)
{
    for(int i=0; i<dim; i++){
        float x = (float) igraph_vector_e(from, i);
        to[i][col] = 1 / x;
...
int main(int argc, char * argv[]){
    ...
    //não sei o que essa funcao realiza
    ...
    ofstream RBEPm("pagerank2seedfixa.csv", std::ios_base::app);
    ...
    geraMatrizCentralidades2(matCentralidades, &pagerank, n, 3);

    igraph_vector_destroy(&pagerank);

    for(int i = 0; i < n; i++){
        matCentralidades[i][4] = vectorBt[i];
    }

    for(int i = 0; i < n; i++){
        matCentralidades[i][5] = dbranch[i];
    }
    ...
    outFile << "N;GRAU;PageRank;PESO;R-BEP;D-BRANCH" << endl;
